...
from combat_functions import max_items, combat, add_enemy
from other_functions import door_code, final_door_fight, game_credits, check_godslayer
from classes import Charachter

def start_story(player, enemies, start_at=0):
    if start_at == 0:
        play_again = chapter0(player)
        return play_again        
    if start_at == 1:
        play_again = chapter1(player, enemies)
        return play_again
    if start_at == 2.1:
        play_again = chapter2_path1(player, enemies)
        return play_again
    if start_at == 2.2:
        play_again = chapter2_path2(player, enemies)
        return play_again
...
def chapter0(player):
    ...
    inquirer_input(slow_text("Are you ready to start?", 0.04), ["Yes"])
    ...
    slow_text(f"{player.getName()} takes his {player.weapon} and enters the forest", 0.04)
    slow_text("I wonder what can be so mysterious about this forest", 0.04)
    save_game(player, 1)
    play_again = "No"
    return play_again

def chapter1(player, enemies):
    ...
    if camp_choice == "Camp":
        ...
        enemies.clear()
        add_enemy(enemies, "Shadow Creature", 1)

        shadow_creature = Charachter(125, 30, 50, 10, 20, 30, "Shadow Creature")
        message = combat(enemies, shadow_creature, player, player.inventory)
    ...
    # Explore route
        ...
        save_game(player, 2.2)
        play_again = "Yes"
        return play_again

def chapter2_path1(player, enemies):
    ...
    if temple_path_choice == "Front":
        ...
        enemies.clear()
        add_enemy(enemies, "Shadow Creature", 2)
        ...
        door_choice = inquirer_input(slow_text("There are two doors in front of you", 0.04), ["1", "2"])

        # If you choose 1
        if door_choice == "1":
            door_code(door_choice, 1, shadow_creature, player, enemies)
            door_choice = inquirer_input(slow_text("There are two doors in front of you", 0.04), ["1", "2"])

            # If you choose 1, 1
            if door_choice == "1":
                door_code(door_choice, 2, shadow_creature, player, enemies)
                door_choice = inquirer_input(slow_text("There are two doors in front of you", 0.04), ["1", "2"])

                # If you choose 1, 1, 1
                if door_choice == "1":
                    door_code(door_choice, 3, shadow_creature, player, enemies)

                    enemies.clear()
                    add_enemy(enemies, "The Final Boss: The Forest Shadow", 1)
    ...
    # Back route
        ...
        add_enemy(enemies, "The Final Boss: The Forest Shadow", 1)
        the_forest_shadow = Charachter(150, 50, 70, 10, 30, 40, "The Final Boss: The Forest Shadow")
        message = combat(enemies, the_forest_shadow, player, player.inventory)
...
def chapter2_path2(player, enemies):
    final_boss = Charachter(225, 50, 85, 15 ,40, 50, "Bartolomeus")
    ...
    if trust_choice == "Yes":
        ...
        message = combat(enemies, shadow_creature_hard, player, player.inventory)
        if message == "you died":
            play_again = inquirer_input(slow_text("Do you want to play again", 0.04), ["Yes", "No"])
            return play_again
        ...
        enemies.clear()
        check_godslayer(enemies, player)
        add_enemy(enemies, "The Final Boss: The Forest Shadow", 1)
        the_forest_shadow_hard = Charachter(200, 50, 80, 10 ,45, 60, "The Final Boss: The Forest Shadow")
        message = combat(enemies, the_forest_shadow_hard, player, player.inventory)
        if message == "you died":
            play_again = inquirer_input(slow_text("Do you want to play again", 0.04), ["Yes", "No"])
            return play_again
        ...
        enemies.clear()
        add_enemy(enemies, "Shadow Creature", 3)
        message = combat(enemies, shadow_creature, player, player.inventory)
        if message == "you died":
            play_again = inquirer_input(slow_text("Do you want to play again", 0.04), ["Yes", "No"])
            return play_again
        ...
        enemies.clear()
        check_godslayer(enemies, player)
        add_enemy(enemies, "The True Final Boss: Bartolomeus", 1)
        true_final_boss = Charachter(225, 50, 85, 15 ,40, 50, "The True Final Boss: Bartolomeus")
        message = combat(enemies, true_final_boss, player, player.inventory)
        if message == "you died":
            play_again = inquirer_input(slow_text("Do you want to play again", 0.04), ["Yes", "No"])
            return play_again
        ...
        enemies.clear()
        add_enemy(enemies, "The Revived True Final Boss: Bartolomeus", 1)
        true_final_boss_revived = Charachter(325, 50, 90, 15 ,50, 70, "The Revived True Final Boss: Bartolomeus")
        dead_or_not = combat(enemies, true_final_boss_revived, player, player.inventory)
        if dead_or_not == "player died to final boss":
            ...
            slow_text("If you have managed to get to this point you have managed to pull off something truly impressive", 0.04)
